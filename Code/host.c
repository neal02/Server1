...
int main() {
    ...
    while (1) {
        ...
        if (gamedata->gameState == 1) {
            ...
            print_selectText(gamedata->stageNum, 1);
            printf("\n\t선택을 입력해주세요 : ");
            scanf("%d", &select);
            int n = write(fd, &select, sizeof(int));
            if (n == -1) {
